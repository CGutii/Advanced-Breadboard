...
class CircuitSimulator:
    ...
    def open_translate_screen(self):
        ...
        #send Matrix
        for row in matrix:
            curr_row = ' '.join(row).encode() + b'\n'
            ser.write(' '.join(row).encode() + b'\n')
            print(curr_row)
            print("Matrix sent.")
        ...
        print("this is the ack that we are getting")
        print(received_data)
        
        #if ack is '2' then continously get sensor data
        if received_data == '2':
        #calling to send out the function
            get_sensor_data()
